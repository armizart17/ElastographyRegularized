...
for it = 1:Nt
    ...
    iStart = (it-1)*Nx + 1;
    iSparse = iStart:iStart+iPeaks(1)-1;
    iA(iSparse) = n*ones(1,iPeaks(1));
    jA(iSparse) = 1:iPeaks(1);
    sA(iSparse) = 1/iPeaks(1);
    B(n) = swsLamda(1);
    n = n+1;
    ...
    for i = 1:length(swsLamda) % For each wavelength
        ...
        iSparse = iStart+iPeaks(i):iStart+iPeaks(i+1)-1;
        iA(iSparse) = n*ones(1,lamdaSamples(i));
        jA(iSparse) = iPeaks(i)+1:iPeaks(i+1);
        sA(iSparse) = 1/lamdaSamples(i);
        B(n) = swsLamda(i); 
        n = n+1;
    end
    ...
    iSparse = iStart+iPeaks(end):iStart+Nx-1;
    iA(iSparse) = n*ones(1,Nx-(iPeaks(end)+1)+1);
    jA(iSparse) = (iPeaks(end)+1):Nx;
    sA(iSparse) = 1/(Nx-(iPeaks(end)+1)+1); 
    B(n) = swsLamda(end); 
    n = n+1;
end
%A(n:end,:) = [];
